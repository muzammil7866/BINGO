...
#include<ctime> //for generating new random numbers each time
...
void mainMenu(); //would allow the user to choose between the menu options and process according to that selection

int gameID(); //Random ID would be assigned to this game

void generateBoard(int board1[][5]); //randomly generates bingo grid numbers 

void displayBoard(int board1[][5], string p, int id); //displays the bingo board

int toss(string p1, string p2); //would randomly toss b/w the both players

void playGame(int board1[][5], int board[][5], int start, string p1, string p2, int id); //the proper function that assigns 0 to the number chosen by either player and also switch their turns

bool winCheck(int board[][5], string p, int id); //would check if a player wins
...
bool winCheck(int board1[][5], string p, int id) {
	int count = 0;
	int lines = 0;
	int score = 0;
	...
	if (lines >= 5)
	{
		...
		{
			fHand <<endl<< id << endl << p << endl << score;
			system("cls");
			cout << "\nCONGRATULATIONS AND BINGO, " << p << " WON with score = "<<score<<"!";
	...
	while (endGame != true)
	{
		...
		else if (mainChoice == 2) //players history file would be called
		{
			...
			while (!fHand.eof())
			{
				...
				cout << "\t"<<iD <<" "<< setw(12) << name << " --> " << setw(5) << score << endl << endl;
		...
		{
			system("cls");
			cout << "\nTHANK YOU FOR PLAYING BINGO 1.0\n" << endl;
			system("pause");
			endGame = true;
		}
		...
		{
			cout << "\nINVALID INPUT, ENTER AGAIN!" << endl<<endl;
			system("pause");
		}
	}
}